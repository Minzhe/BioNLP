...
# convert passage level annotations in SeeDev data to sentence level data points
...
class clsEntity:
	...
	def createEntityFromString(line):
		try :
			...
			print("Problem in createEntityFromString ", line, file=sys.stderr)
			sys.exit(-1)
...
class coreNLP:
	...
	def parse(self, fname):
		...
		if len(self.documentId) > 0:
			print("Multiple parse calls on nlp obj.", file=sys.stderr)
			sys.exit(-1)			
		...
		for sent in re.split(r"Sentence #\d+ \(\d+ tokens\):", fdata):
			# loop through each parsed sentence
			if not sent or len(sent) <= 0: 
				continue # how do we even get empty lines ??
			...
			tokboundaries = []
			sentenceBoundary = 0
			for tokinfo in re.findall(r"\[Text=(.*) CharacterOffsetBegin=(\d+) CharacterOffsetEnd=(\d+) PartOfSpeech=(.*) Lemma=(.*) NamedEntityTag=.*\]" ,sent):
			...
			m = re.search(r"(\(ROOT.*\n\n)root\(", sent, re.DOTALL)
			if not m :
				print("Failed to extract parse info for sentence no.", sentNo, file=sys.stderr) 
				sys.exit(-1)
			else:
				pt = m.group(1)
				self.parseTree.append(re.sub(r"\s+", " ", pt))

			# get the dependency graph info
			m = re.search(r"(\nroot\(.*\))",sent, re.DOTALL)
			if not m:
				print("Failed to get dependency graph for sentence no.", sentNo, file=sys.stderr)
				sys.exit(-1)
			else:
				pt = m.group(1)
				self.dependencyGraph.append(re.sub(r"\s+", " ", pt)) 
...
def get_doc_obj(e1,e2):
	...
	if e1.sentenceId != e2.sentenceId or e1.documentId != e2.documentId:
		print("Problem cross sentence entity pairs in get_doc_obj", e1.get_display(), e2.get_display(), file=sys.stderr)
		sys.exit(-1)
